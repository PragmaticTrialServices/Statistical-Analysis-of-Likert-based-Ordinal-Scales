...
generate_npo_effects <- function(K, beta_1, nonPO, attenuateeffect) {
  ...
  if (K == 11) { # Logic for 11 categories (outcome 0-10)
    # Per prompt: PO violation is δ1=0; δ2–6=0.15; δ7–10=0.30
    effect_val1 <- 0.15
    effect_val2 <- 0.30
    ...
    # Prompt: δ_2 to δ_6 = 0.15 => Corresponds to R indices 2 through 6
    npo_effects[2:6] <- effect_val1
    # Prompt: δ_7 to δ_10 = 0.30 => Corresponds to R indices 7 through 10
...
run_one_scenario <- function(beta_1, nonPO, atten, sym_flag, n, n_simlist = 1000,
  ...
  idx_npo_any_pow   <- 26    # "any significant" flag
  ...
  if (runvglm == 1) {
    ...
    npo_any_pow <- mean(raw[, idx_npo_any_pow], na.rm = TRUE) # Calculate power 
                                                              # based on "any 
                                                              # significant" flag
...
run_simulation_grid <- function(
  ...
  save_csv_with_fallback(summary_df, "simulation_summary_grid_strong_po_violation.csv")
  #save_csv_with_fallback(power_df,   "simulation_power_grid_strong_po_violation.csv")
...
run_simulation_grid(beta_1list          = c(log(0.5),log(0.6),log(0.7),log(0.8),log(0.9),log(1.0)),
                    ...
                    n_simlist           = 1000, # Use 100 for a quick test run
                    ybinhigh            = 7,
                    runvglm             = 1,
                    seed                = 14424)
